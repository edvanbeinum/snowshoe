...
class Navigation
{
    ...
    /**
     * @todo use setter injection to mock this for testing
     *
     * @var \Husky\Parser\Parser
     */
    protected $_parser;

    /**
     *
     * @param \Husky\Parser\Parser $parser - needed to generate the page Titles
     */
    ...
    public function getPrimaryNavigation(array $fileInfoArray)
    {
        ...
        foreach ($fileInfoArray as $contentFileInfo) {
            $links[] = array(
                ...
                'title' => $this->_parser->getPageTitle($contentFileInfo)
            );
        }
        return $links;
    }
